...
def get_sample_audit() -> pd.DataFrame:
    rows = []
    halls = [
        ("Independence", "Block C", 160),
        ("Tedder",        "Floor 1",  60),
        ("Awo",           "Floor 1",  40),
    ]
    bio_base    = {"Independence": 3.5, "Tedder": 2.5, "Awo": 6.5}
    nonbio_base = {"Independence": 5.0, "Tedder": 4.5, "Awo": 4.2}
    rec_base    = {"Independence": 1.2, "Tedder": 0.7, "Awo": 1.8}
    haz_base    = {"Independence": 0.0, "Tedder": 0.4, "Awo": 0.7}
    for hall, floor, pop in halls:
        for day in range(1, 8):
            mult = 1.0 + 0.15 * max(0, day - 4)
            rows.append({
                "hall": hall, "floor_block": floor,
                "date": f"2025-05-0{day}", "day_number": day,
                "biodegradable_kg":     round(bio_base[hall]    * mult + np.random.uniform(-0.2, 0.2), 2),
                "non_biodegradable_kg": round(nonbio_base[hall] * mult + np.random.uniform(-0.2, 0.2), 2),
                "recyclable_kg":        round(rec_base[hall]    * mult + np.random.uniform(-0.1, 0.1), 2),
                "hazardous_kg":         round(haz_base[hall]    * mult + np.random.uniform(-0.05, 0.05), 2),
                "population": pop,
            })
    return pd.DataFrame(rows)


def get_sample_survey() -> pd.DataFrame:
    np.random.seed(42)
    n = 18
    visitor = np.random.randint(0, 2, n)
    waste   = np.clip(np.round(1.5 + 1.2 * visitor + np.random.normal(0, 0.4, n)), 1, 3).astype(int)
    return pd.DataFrame({
        "respondent_id":              range(1, n + 1),
        "hall":                       np.random.choice(["Tedder", "Independence", "Awo"], n),
        "level":                      np.random.choice([100, 200, 300, 400, 500], n),
        "meals_per_day":              np.random.choice([1, 2, 3], n),
        "packaged_food_frequency":    np.random.choice([0, 1, 2], n),
        "disposal_frequency_per_week":np.random.randint(1, 8, n),
        "recycling_awareness":        np.random.choice([1, 2, 3, 4, 5], n,
                                         p=[0.22, 0.33, 0.28, 0.11, 0.06]),
        "visitor_effect":             visitor,
        "separation_behaviour":       np.random.choice([0, 1, 2, 3, 4], n),
        "waste_scale":                waste,
    })
...
def sidebar():
    ...
    return page, audit_file, survey_file, use_sample
...
def page_overview(audit_df, survey_df):
    ...
    for col, (icon, title, desc) in zip(cols, features):
        ...
        <div style="background:#161b27; border:1px solid #21262d; border-radius:12px;
                    padding:24px; height:160px; box-shadow:0 4px 20px rgba(0,0,0,0.3);">
            <div style="font-size:1.6rem; margin-bottom:10px;">{icon}</div>
            <div style="font-weight:700; color:#e2e8f0; margin-bottom:8px; font-size:0.95rem;">{title}</div>
            <div style="color:#718096; font-size:0.8rem; line-height:1.5;">{desc}</div>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    page, audit_file, survey_file, use_sample = sidebar()

    # Resolve data sources
    if audit_file:
        audit_df = load_df(audit_file)
    elif use_sample:
        audit_df = get_sample_audit()
    else:
        audit_df = None
